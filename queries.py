...
def account_txn_trend(client_id):
    query = f"""
    SELECT EXTRACT(YEAR FROM t.date) AS txn_year, COUNT(*) AS txns
    FROM transactions_data t
    WHERE t.client_id = '{client_id}'
    GROUP BY txn_year
    ORDER BY txn_year;
...
def fraud_by_mcc_user(client_id):
    query = f"""
    SELECT m.description AS mcc_category, COUNT(*) AS frauds
    FROM transactions_data t
    JOIN train_fraud_labels f ON t.id = f.id
...
def fraud_trend_yearly(client_id):
    query = f"""
    SELECT EXTRACT(YEAR FROM t.date) AS year, COUNT(*) AS frauds
    FROM transactions_data t
    JOIN train_fraud_labels f ON t.id = f.id
    WHERE t.client_id = '{client_id}' AND f.target = 'Yes'
    GROUP BY year
...
def dashboard2_userwise_summary(client_id):
    ...
    return {
        ...
        "fraud_by_card_type": fraud_by_mcc_user(client_id),
        "fraud_by_card_brand": fraud_trend_yearly(client_id),
